...
int is_leaf_node(node_t* ptr) {
  ...
  if((ptr->right) == NULL && (ptr->left) == NULL){// it has no children
    return 1;
  }
  return 0;
}
...
int is_internal_node(node_t* ptr) {
  // TODO: (remember to modify the return value appropriately)
  if((ptr->right) != NULL || (ptr->left) != NULL ){//has a child on either left or right
    return 1;
  }
  return 0;
}
...
int is_in_subtree(node_t* a, node_t* b) {
  ...
  left = a;
  right = a;
while(is_internal_node(left)){
  if((left->left == b) || (left->right ==b)){
    return 1;
  }
  left = left->left;
}
while(is_internal_node(right)){
  if((right->right == b) || (right->left == b)){
    return 1;
  }
  right = right->right;
}
...
void rotate(node_t* ptr) {
  // TODO:
  int heightSave = ptr->module->h; //safe the calue of height
  ptr->module->h = ptr->module->w; //swap the width to height location
  ptr->module->w = heightSave; //load the saved height to width
}
...
void recut(node_t* ptr) {
  ...
  // TODO:
  if(ptr->cutline == V){//if it is a vertical cut change it
    ptr->cutline = H;//to a horizontal cut
  }else{
    ptr->cutline = V; //else change to verticcal cut
  }
  return;
}
...
void swap_module(node_t* a, node_t* b) {
  ...
  // TODO:
  module_t *aSave = a->module;// save the module
  a->module = b->module;//change the value in a to b
  b->module = aSave;//update the value in b to module of a
}
...
void swap_topology(node_t* a, node_t* b) {
  ...
  node_t *aSave = a;
  node_t *bSave = b;
  node_t *bparent = b->parent; //safe the parent for b
  node_t *aparent = a->parent; //safe the parent for a
  //check the location of a
  if(aparent->left == a){//check if a is the left or the right child
    aparent->left = b;//update the location of b
    b->parent = aparent;//update the parent location
  }
  if(aparent ->right == a){
    aparent ->right =b;
    b->parent = aparent;
  }
  if(bparent->left == bSave){//check if b is the left or the right child
    bparent->left = aSave;//update the location of a
    a->parent = bparent;//update the parent location
  }
  if(bparent->right == bSave){
    bparent->right = aSave;
    a->parent = bparent;
  }
}
...
void postfix_traversal(node_t* ptr, int* nth, expression_unit_t* expression) {
  ...
  // TODO:
  postfix_traversal(ptr->left,nth,expression);//traverse through the leftsubtree
  postfix_traversal(ptr->right,nth,expression);//traverse through the rightsubtree

  if(ptr->module != NULL){// if module pointer exist
    expression[*nth].cutline = UNDEFINED_CUTLINE;//update the cutline to NULL
    expression[*nth].module = ptr->module;
  }
  if(ptr->cutline != UNDEFINED_CUTLINE){//if it is a cutline type
    expression[*nth].module = NULL;//update the module to n=NULL
...
int sum = 0;
int i;
for(i=0;i<num_modules;i++){//loop through the module
  sum = sum + modules[i].resource;//add the sum
}
  return sum;
}
...
node_t* init_slicing_tree(node_t* par, int n) {
  ...
  if(n == num_modules-1){//base case
    node_t *leftp = (node_t*)malloc(sizeof(node_t));
    node_t *rightp = (node_t*)malloc(sizeof(node_t));

    par->left = leftp; //set up the pointer
    par->right = rightp;

    leftp->module = &modules[n]; //set the value to the pointer
    rightp->module = &modules[n-1];

    leftp->cutline = UNDEFINED_CUTLINE;//set up the cutline
    rightp->cutline = UNDEFINED_CUTLINE;

    leftp->parent = par; //set the parent
    rightp->parent = par;

    leftp->left = NULL;//set the children pointer
    leftp->right = NULL;
    rightp->left = NULL;
    rightp->right = NULL;
  ...
  if(par != NULL){//case where could add a right leaf
    node_t *rightp = (node_t*)malloc(sizeof(node_t));//set up the right pointer value
    rightp->cutline = UNDEFINED_CUTLINE;
    rightp->module = &modules[n-1];
    rightp->left = NULL;
    rightp->right = NULL;
    rightp->parent = par;//set up the prent relationship
    par->right = rightp;
    par->left = leftp;
  }
  init_slicing_tree(leftp,(n+1));//recursive part
